...
POC 自动化漏洞扫描器 v2.4 - 攻击面检测 + 漏洞挖掘 + 模拟攻击测试

## 功能特性

- **自动化 POC 扫描**：基于 Markdown 格式的 POC 文件自动执行漏洞检测
- **攻击面检测**：全面扫描系统潜在的安全暴露点（路径、文件、HTTP 方法）
- **智能漏洞挖掘**：发现攻击面后自动使用 POC 进行深度验证
- **模拟攻击测试**：通过精心构造的 Payload 进行渗透测试
- **动态规则引擎**：结合指纹验证机制，提升检测准确性
...
| 命令 | 说明 |
|------|------|
| `scan` | 开始扫描（单目标或批量，**自动并行攻击面检测**） |
| `attack` | 单独执行攻击面检测与 POC 验证 |
...
启动并行攻击面检测...
使用 POC 进行攻击面验证...
对 /admin 执行 2 个 POC 测试
对 /login 执行 3 个 POC 测试
攻击面 POC 验证发现 5 个漏洞
==================================================
完成 | 120s | 总漏洞：20 (POC: 15, 攻击面：5) | 50 跳过
```
...
高风险发现：5 个
POC 验证成功的漏洞:
  - attack_surface_weak_password: http://target.com/admin
  - attack_surface_sql_injection: http://target.com/login
  - attack_surface_auth_bypass: http://target.com/phpmyadmin
```
...
| `enable_attack_surface` | **启用攻击面检测** | true |
| `attack_surface_threads` | **攻击面检测线程数** | 10 |
...
- 扫描基本信息（目标、时间、耗时）
- 指纹识别结果
- 漏洞汇总统计（POC 漏洞 + 攻击面挖掘漏洞）
- 漏洞详情（POC 名称、请求方法、URL、状态码、置信度、Payload）
- **攻击面检测专区**（风险点、风险等级、验证结果）
- 批量扫描结果（如适用）
...
1. **路径扫描**：检测 30+ 常见敏感路径（admin、login、.git、.env 等）
2. **文件扫描**：扫描备份文件、配置文件、数据库文件等
3. **方法扫描**：测试 PUT、DELETE、OPTIONS、TRACE 等 HTTP 方法
4. **智能映射**：根据路径特征匹配漏洞类型
5. **POC 调用**：自动加载对应类型的 POC 进行测试
6. **验证确认**：使用动态规则引擎验证漏洞
7. **报告生成**：整合 POC 扫描和攻击面挖掘结果
...
4. **误报处理**：高置信度漏洞建议人工复核
5. **攻击面检测**：默认启用，可通过配置文件禁用

## 更新日志

### v2.4
- **新增攻击面检测功能**：全面扫描系统潜在安全暴露点
- **深度 POC 整合**：发现攻击面后自动使用 POC 进行验证
- **并行执行架构**：POC 扫描与攻击面检测同时执行
- **智能漏洞映射**：根据路径特征自动匹配漏洞类型
- **增强报告功能**：攻击面检测专区、风险等级分类
- **动态规则引擎**：内置 12+ 种漏洞类型测试规则
- **模拟攻击测试**：认证绕过、权限提升、数据泄露场景
